    ...
    <div className="pl-6 sm:pl-10">
      <div
        className={`relative flex flex-row flex-grow-0 w-full p-2 cursor-pointer rounded-l-full hover:bg-secondary ${
          props.index === props.currentSelected ? "bg-secondary-hover" : ""
        }`}
      ...
      >
        ...
        {(
          ...
          <img
            src={`${NEXT_PUBLIC_BACKEND_URL}/api/${
              props.data.UserOneId !== props.currentUser?.user.id
                ? props.data.UserOnePicture
                : props.data.UserTwoPicture
            }`}
            alt="profilePicture"
        ...
        <div className="flex flex-col justify-center flex-grow ml-4 text-tertiary">
          <span className="text-2xl tracking-wide truncate w-4/5">
            {props.data.UserOneId !== props.currentUser?.user.id ? props.data.UserOneName : props.data.UserTwoName}
          </span>
          <div className="grid grid-cols-1">
            {props.data.lastMessage ? (
              <span className="text-sm truncate">{props.data.lastMessage}</span>
            ) : (
              <span className="text-sm text-secondary italic">Pas de message</span>
            )}
          </div>
        </div>
        {props.index === props.currentSelected && <div className="absolute right-0 top-0 w-1 h-full bg-background" />}
      </div>
    </div>
  );
  {
