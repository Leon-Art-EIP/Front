...
export default function Gallery4x4(props: Gallery4x4Props): JSX.Element {
  ...
  return (
    <div className="grid xl:grid-cols-4 lg:grid-cols-3 grid-cols-2 gap-8 place-items-center">
      {props.artPublications.artPublications.map((artPublication, index) => (
        <div
          key={index}
          className="relative group w-full aspect-square rounded-xl overflow-hidden hover:scale-[1.03] hover:opacity-75 duration-300 hover:shadow-2xl transition ease-in-out cursor-pointer"
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 w-full h-32 opacity-0 group-hover:opacity-100">
            ...
            <div className="absolute bottom-0 left-0 p-2 w-full text-white">
              <p className="text-lg font-semibold text-white">{artPublication.name}</p>
            </div>
          </div>
        </div>
      ))}
