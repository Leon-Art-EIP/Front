  ...
  return (
    <div className="flex flex-row gap-8 justify-start overflow-x-auto pb-3">
      {props.users.users.map((user, index) => (
        <div
          key={index}
          className="flex flex-col group cursor-pointer rounded-xl hover:opacity-75 transition-opacity hover:bg-secondary hover:shadow-lg duration-300 ease-in-out px-4 py-2"
        ...
        >
          ...
          <span className="text-xl font-semibold text-center group-hover:opacity-100 text-tertiary">
            {user.username}
          </span>
        </div>
      ))}
