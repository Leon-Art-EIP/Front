    ...
    <div className="flex gap-1">
      {Array.from(Array(props.nbrPoints).keys()).map((_, index) => {
        if (index + 1 === props.position) {
          return <CircleTwoTone key={`point-${index}`} className="text-blue-300" style={{ fontSize: 12 }} />;
        }
        return <Circle key={`point-${index}`} className="text-secondaryGreyDisabled" style={{ fontSize: 12 }} />;
      })}
    </div>
  );
}
