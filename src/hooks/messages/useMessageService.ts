...
export function useMessageService(conversationService: ConversationService): MessageService {
  ...
  async function fetchOrderInfo(convId: number | undefined) {
    const res = await fetch(BACKEND_URL + "/api/conversations/order/infos", {
      method: "POST",
      body: JSON.stringify({
        convId: convId,
      }),
  ...
  async function fetchMessages(convId: number | undefined) {
    const res = await fetch(BACKEND_URL + "/api/conversations/messages", {
      method: "POST",
      body: JSON.stringify({
        convId: convId,
      }),
  ...
  async function sendMessage(convId: number | undefined, messageToSend: string) {
    const res = await fetch(BACKEND_URL + "/api/conversations/messages/new", {
      method: "POST",
      body: JSON.stringify({
        convId: convId,
        contentType: "text",
  ...
  async function handleRating(convId: number | undefined, rating: number) {
    ...
    const res = await fetch(BACKEND_URL + "/api/conversations/order/rating", {
      method: "POST",
      body: JSON.stringify({
        convId: convId,
        rating: rating,
